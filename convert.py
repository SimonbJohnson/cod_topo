...
listOfAtts = ['admin{level}RefName','Admin{level}Name','admin{level}Name_en','admin{level}Name_fr','admin{level}Name_es','admin{level}Name_pt','admin{level}Name_ar']
...
for link in links:
	...
	geojson = 'geoms/geojson/'+country+'/'+str(level)+'/geom.geojson'
	topojson = 'geoms/topojson/'+country+'/'+str(level)+'/geom.json'
	try:
		with open(geojson) as json_file:
			...
			newFile = 'geoms/geojson/'+country+'/'+str(level)+'/geom_modified.geojson'

			with open(newFile, 'w') as outfile:
				json.dump(geojsonFile, outfile)

		if not os.path.exists('geoms/topojson/'+country+'/'+str(level)):
		 	os.makedirs('geoms/topojson/'+country+'/'+str(level))
